...
def parse_args():
    ...
    parser.add_argument("--dataset", type=str, default="cnn_dailymail",
                        choices=["cnn_dailymail", "gov_report", "qmsum", "vcsum"],
                        help="Dataset to evaluate on (default: cnn_dailymail)")
    ...
    parser.add_argument("--max-seq-len", type=int, default=None,
                        help="Override max sequence length (default: dataset-specific)")
    parser.add_argument("--cache-size", type=int, default=None,
                        help="Override KV cache size (default: dataset-specific)")
...
def load_dataset_config(dataset_name):
    """Load dataset and configuration based on dataset name.
    
    Each dataset config includes:
    - loader: function to load the dataset
    - text_field: field name for input text
    - summary_field: field name for reference summary/answers
    - id_field: field name for sample ID
    - prompt_template: prompt format with {text} placeholder
    - task_type: 'summarization' or 'qa'
    - max_seq_len: maximum sequence length for this dataset
    - cache_size: KV cache maximum size for this dataset
    """
    configs = {
        "cnn_dailymail": {
...
dataset_cfg = load_dataset_config(DATASET_ARG)

# Extract dataset-specific sequence length and cache size
...
if CACHE_SIZE_OVERRIDE is not None:
    CACHE_SIZE = CACHE_SIZE_OVERRIDE
...
def compute_char_level_rouge(pred, ref):
    """Compute character-level ROUGE-L (for Chinese and CJK languages)."""
...
try:
    ...
    def compute_rouge_l(pred, ref):
        ...
        if is_chinese(ref):
            return compute_char_level_rouge(pred, ref)
        # For English, use word-level matching
    ...
    print("ROUGE-L: manual LCS ✓ (with character-level support for Chinese)")
...
_rouge = compute_rouge_l(_pred, _ref_str)
print(f"  Reference:  {_ref_str[:80]}...")
print(f"  Prediction: {_pred[:80]}...")
print(f"  Prediction raw: '{_pred}'")
print(f"  ROUGE-L:    {_rouge:.4f}  ✓\n")
print("Generating visualizations...")
_test_sample = ds[SPLIT][0]
_test_text = _test_sample[dataset_cfg["text_field"]]
_test_prompt = dataset_cfg["prompt_template"].format(text=_test_text)
visualize_and_log(
    model, tokenizer,
    sample      = {"article": _test_text, "highlights": _test_sample[dataset_cfg["summary_field"]]},
    kv_mode     = KV_MODE,
    kv_cfg      = kv_cfg,
    cache_size  = CACHE_SIZE,
    max_seq_len = MAX_SEQ_LEN,
    prefix      = f"viz_{DATASET_ARG}_{KV_MODE}_{H2O_STRATEGY}",
)
OUTPUT_FILE     = f"{DATASET_ARG}_{KV_MODE}_{H2O_STRATEGY}_{SPLIT}_{NUM_SAMPLES}.json"
CHECKPOINT_FILE = f"checkpoint_{DATASET_ARG}_{KV_MODE}_{H2O_STRATEGY}_{SPLIT}.json"
SAVE_EVERY      = 100
print(f"{'='*60}")
...
for idx in tqdm(range(start_idx, NUM_SAMPLES), desc=f"[{KV_MODE}|{H2O_STRATEGY}]"):
    if reset_scores is not None:
        reset_scores()
    sample    = ds[SPLIT][idx]
    text      = sample[dataset_cfg["text_field"]]
    reference = sample[dataset_cfg["summary_field"]]
    # Handle both list and string reference formats
    reference_str = reference[0] if isinstance(reference, list) and reference else str(reference)
    sample_id = sample.get(dataset_cfg["id_field"], idx)
    prompt = dataset_cfg["prompt_template"].format(text=text)
    ...
    rouge         = compute_rouge_l(prediction, reference_str)
...
print(f"\n{'='*60}")
print(f"DONE — Dataset: {DATASET_ARG}  KV Mode: {KV_MODE}  Strategy: {H2O_STRATEGY}")
